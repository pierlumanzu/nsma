...
from Algorithms.Genetic.Genetic_Utils.Survival_Strategy import Genetic_Survival_Strategy


class Memetic_Survival_Strategy(Genetic_Survival_Strategy):
    """
    This class implements the survival strategy of NSMA. It is very similar to the one of NSGA-II (see Genetic_Survival_Strategy.py).
    ...
    def __init__(self, pop_size: int, crowding_quantile: float):
        ...
        Genetic_Survival_Strategy.__init__(self, pop_size)

        self.__crowding_quantile = crowding_quantile

    def leastInfeasibleSurvival(self, p_list: np.array, f_list: np.array, constraint_violations: np.array, n_survive: int, eps_pareto_stationarity: np.array = None):
        """
        Use the infeasible solutions survival strategy
    ...
    def rankingAndCrowdingFeasibleSurvival(self, p_list: np.array, f_list: np.array, constraint_violations: np.array, n_survive: int, eps_pareto_stationarity: np.array = None):
        """
        Use the feasible solutions survival strategy
        :param p_list: problem solutions
        :param f_list: related points in the objectives space
    ...
    def getSurvivals(self, p_list: np.array, f_list: np.array, constraint_violations: np.array, eps_pareto_stationarity: np.array = None):
        """
        Execute the survival strategy
        :param p_list: problem solutions
        :param f_list: related points in the objectives space
        ...
        Notes:  It divides the solutions in feasible and infeasible ones through the function splitByFeasibility(np.array)
                If there are no feasible solutions, then the function leastInfeasibleSurvival(np.array, np.array, np.array, int, np.array) is called to get the survivals among the infeasible points
                Otherwise, the function rankingAndCrowdingFeasibleSurvival(np.array, np.array, np.array, int, np.array) is first called to get the survivals among the feasible solutions
                Then, the function leastInfeasibleSurvival(np.array, np.array, np.array, int, np.array) is called to get the last survivals among the infeasible points
                ...
                The function raises an AssertionError if the parameter eps_pareto_stationarity is not defined
                The epsilon value for the epsilon-Pareto-stationarity is a NSMA parameter (theta_for_stationarity; see NSMA.py).
                If the function rankingAndCrowdingFeasibleSurvival(np.array, np.array, np.array, int, np.array) is not called, the threshold_crowding_distance variable remains undefined
        """
        ...
        if len(feasible) == 0:
            p_list_inf, f_list_inf, constraint_violations_inf, eps_pareto_stationarity_inf = self.leastInfeasibleSurvival(p_list[infeasible], f_list[infeasible], constraint_violations[infeasible], min(len(infeasible), n_survive), eps_pareto_stationarity[infeasible])
        else:
            p_list_f, f_list_f, constraint_violations_f, rank_list_f, crowding_list_f, eps_pareto_stationarity_f, threshold_crowding_distance = self.rankingAndCrowdingFeasibleSurvival(p_list[feasible], f_list[feasible], constraint_violations[feasible], min(len(feasible), n_survive), eps_pareto_stationarity[feasible])

            n_infeasible_survive = n_survive - len(p_list_f)

            if n_infeasible_survive > 0:
                p_list_inf, f_list_inf, constraint_violations_inf, eps_pareto_stationarity_inf = self.leastInfeasibleSurvival(p_list[infeasible], f_list[infeasible], constraint_violations[infeasible], min(len(infeasible), n_infeasible_survive), eps_pareto_stationarity[infeasible])
