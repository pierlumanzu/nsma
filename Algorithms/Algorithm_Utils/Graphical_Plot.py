...
class Graphical_Plot:
    ...
    def figureCreation(self, figure_name: str, figure_size: tuple[float, float] = None):
        """
        Create a figure
    ...
    def initSingleFigure(self, dim: int, figure_name: str):
        """
        Initialize a single figure for the plots
        ...
        Notes:  It raises an AssertionError if the plot dimension is not in {2, 3}.
                It calls the function figureCreation(str, tuple[float, float]) to create the figure.
        ...
        self.figureCreation(figure_name)
    ...
    def initMainFigure(self, n: int, m: int):
        """
        Initialize the requested figures for the plots
        ...
        Notes:  It raises an AssertionError if the front dimension is not in {2, 3}.
                ...
                It calls:
                    - the function initSingleFigure(int, str), if the Pareto solutions plot is not requested or not possible to show;
                    - the function figureCreation(str, tuple[float, float]), if the Pareto solutions plot is requested and its dimension is in {2, 3}.
        ...
        if not self.__plot_pareto_solutions or n > 3 or n == 1:
            self.initSingleFigure(m, 'Pareto front')
        else:
            self.figureCreation('', figure_size=(14.5, 4.8))
    ...
    def saveFigure(self, p_list: np.array, f_list: np.array, file_base_path: str, algorithm_name: str, problem_name: str):
        """
        Save the figure
        :param p_list: Pareto solutions
        :param f_list: Pareto front points
        ...
        if not self.__plot_pareto_solutions or n > 3 or n == 1:
            ...
            self.initSingleFigure(n, 'Pareto solutions')
