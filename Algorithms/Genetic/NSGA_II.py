...
from Algorithms.Genetic.Genetic_Utils.General_Utils import binaryTournament, setToBoundsIfOutside, removeListsDuplicatesPoint
...
class NSGA_II(Genetic_Algorithm):
    """
    ...
    def search(self, p_list: np.array, f_list: np.array, problem: Problem):
        ...
        p_list, f_list, constraint_violations, rank_list, crowding_list = self._survival_strategy.getSurvivals(p_list, f_list, constraint_violations)

        self.showFigure(p_list, f_list)

        while not self.evaluateStoppingConditions():

            # Verbose_System: the metrics specific for NSGA-II are the maximum finite rank, the maximum rank (including the infinity values) and the minimum crowding distance. #
            ...
            p_list_off = self.getOffsprings(p_list, f_list, constraint_violations, crowding_list, problem)
    ...
    def getParents(self, p_list: np.array, f_list: np.array, constraint_violations: np.array, crowding_list: np.array, n_offspring: int = None):
        """
        Get the solution parents
        :param p_list: problem solutions
        :param f_list: related points in the objectives space
        ...
        Notes:  It raises an AssertionError if the parameter n_offspring is not defined.
                After randomly selecting the solution couples, it calls the function binaryTournament(np.array, np.array, np.array, np.array) to select a parent from each pair
        ...
        S = binaryTournament(f_list, constraint_violations, crowding_list, P)

        return np.reshape(S, (n_select, 2))

    def calcBetaQ(self, beta: np.array, rand: np.array):
        """
    ...
    def innerCrossover(self, p_list_parents: np.array, problem: Problem, surrogate_lb: np.array = None, surrogate_ub: np.array = None):
        """
        See the function crossover(np.array, np.array, Problem, np.array, np.array)
        ...
        beta_q = self.calcBetaQ(beta, rand)
        ...
        c[0] = setToBoundsIfOutside(c[0], x_l, x_u)
        c[1] = setToBoundsIfOutside(c[1], x_l, x_u)
    ...
    def crossover(self, p_list: np.array = None, parents: np.array = None, problem: Problem = None, surrogate_lb: np.array = None, surrogate_ub: np.array = None):
        ...
        _p_list_offsprings = self.innerCrossover(p_list_parents, problem, surrogate_lb, surrogate_ub)
    ...
    def mutation(self, off: np.array = None, problem: Problem = None, surrogate_lb: np.array = None, surrogate_ub: np.array = None):
        ...
        Y = setToBoundsIfOutside(Y, surrogate_lb if surrogate_lb is not None else problem.filtered_lb, surrogate_ub if surrogate_ub is not None else problem.filtered_ub)
    ...
    def getOffsprings(self, p_list: np.array, f_list: np.array, constraint_violations: np.array, crowding_list: np.array, problem: Problem, surrogate_lb: np.array = None, surrogate_ub: np.array = None):
        """
        Get the solution offsprings
        :param p_list: problem solutions
        :param f_list: related points in the objectives space
        ...
        Notes:  It can handle the surrogate bounds instead of the original ones (see NSMA.py).
                It calls the functions getParents(np.array, np.array, np.array, np.array, int), crossover(np.array, np.array, Problem, np.array, np.array)
                ...
                In each iteration, the offsprings that are duplicates of other ones or of solutions in the current population are removed through the removeListsDuplicatesPoint(np.array, Object) function.
        """
        ...
        while len(off) < self._pop_size:
            ...
            parents = self.getParents(p_list, f_list, constraint_violations, crowding_list, n_remaining)
            ...
            _off = removeListsDuplicatesPoint(_off, p_list, off)
