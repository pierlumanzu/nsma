...
def binaryTournament(f_list: np.array, constraint_violations: np.array, crowding_list: np.array, P: np.array):
    """
...
def calcDominationMatrix(f_list: np.array):
    """
...
def fastNonDominatedSort(f_list: np.array):
    """
    Divide the solutions in fronts
    :param f_list: solutions values in the objectives space
    :return: the fronts

    Notes: It computes the domination matrix through the function calcDominationMatrix(np.array)
...
def nonDominatedSorting(f_list: np.array, n_stop_if_ranked: int):
    """
    ...
    Notes: It calls the function fastNonDominatedSort(np.array) to associate each solution to a rank and a front.
    ...
    fronts = fastNonDominatedSort(f_list)
...
def splitByFeasibility(constraint_violations: np.array):
    """
...
def setToBoundsIfOutside(X: np.array, xl: np.array, xu: np.array):
    """
...
def findDuplicates(p_list: np.array, epsilon: float = 1e-24):
    """
...
def calcCrowdingDistance(f_list: np.array, filter_out_duplicates: bool = True):
    """
    Calculate the crowding distance for each solution
    :param f_list: solutions values in the objectives space
    :param filter_out_duplicates: if set to True, it does not consider the duplicates in the crowding distance computation; in order to find these, the function findDuplicates(np.array) is employed
    ...
    if n_points <= 2:
        ...
        if filter_out_duplicates:
            is_unique = np.where(np.logical_not(findDuplicates(f_list)))[0]
        else:
...
def removeDuplicatesPoint(p_list_1: np.array, f_list_1: np.array = None, constraint_violations_1: np.array = None, p_list_2: np.array = None, epsilon: float = 1e-16):
    """
...
def removeListsDuplicatesPoint(p_list: np.array, *args):
    """
    ...
    p_list = removeDuplicatesPoint(p_list)
    for arg in args:
        if len(arg) > 0:
            if len(p_list) == 0:
                break
            else:
                p_list = removeDuplicatesPoint(p_list, p_list_2=arg)
