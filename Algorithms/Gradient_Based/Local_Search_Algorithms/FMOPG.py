...
class FMOPG(Gradient_Based_Algorithm):
    """
    ...
    def __init__(self,
        ...
        Gradient_Based_Algorithm.__init__(self,
                                          ...
                                          name_DDS='Boundconstrained_Projected_Gradient_DDS', name_ALS='Boundconstrained_Front_ALS')

        self.__theta_array = np.array([-np.inf], dtype=float)
        Gradient_Based_Algorithm.addStoppingCondition(self, 'theta_tolerance', theta_tol, self.__theta_array[0], equal_required=True)
        ...
        Gradient_Based_Algorithm.addStoppingCondition(self, 'min_alpha', 0, self.__alpha_array[0], smaller_value_required=True, equal_required=True)
    ...
    def search(self, p_list: np.array, f_list: np.array, problem: Problem, index_initial_point: int = None, I: tuple = None):
        ...
        while not self.evaluateStoppingConditions():
            ...
            n_iteration = self.getStoppingConditionCurrentValue('max_iter')
            ...
            v, theta = self._direction_solver.computeDirection(problem, J[I, ], x_p=p_list[index_point, :])
            self.__theta_array[n_iteration] = theta
            self.updateStoppingConditionCurrentValue('theta_tolerance', self.__theta_array[n_iteration])
            ...
            if theta < self._theta_tol:
                ...
                self.updateStoppingConditionCurrentValue('min_alpha', self.__alpha_array[n_iteration])
                ...
                if new_p is not None:
                    ...
                    self.updateStoppingConditionCurrentValue('theta_tolerance', self.__theta_array[n_iteration + 1])
                    ...
                    self.updateStoppingConditionCurrentValue('min_alpha', self.__alpha_array[n_iteration + 1])

            self.addToStoppingConditionCurrentValue('max_iter', 1)

        return p_list, f_list, self.__theta_array

    def resetStoppingConditionsCurrentValues(self, theta_tol: float):
        """
        Reset the stopping conditions current values
        ...
        self.updateStoppingConditionCurrentValue('max_iter', 0)

        self._theta_tol = theta_tol
        self.__theta_array = np.array([-np.inf], dtype=float)
        self.updateStoppingConditionCurrentValue('theta_tolerance', self.__theta_array[0])
        self.updateStoppingConditionReferenceValue('theta_tolerance', theta_tol)

        self.__alpha_array = np.array([1], dtype=float)
        self.updateStoppingConditionCurrentValue('min_alpha', self.__alpha_array[0])
