...
from Line_Searches.Line_Search_Utils.Pareto_Utils import existsSufficientlyDominatingPoint

from Problems.Problem import Problem


class Boundconstrained_Front_ALS(ALS):
    """
    ...
    def search(self, problem: Problem, x: np.array, f_list: np.array, d: np.array, theta: float, I: np.array):
        ...
        while (not problem.checkPointFeasibility(new_x) or np.isnan(new_f).any() or np.isinf(new_f).any() or existsSufficientlyDominatingPoint(new_f[I], f_list[:, I], alpha, self._beta, theta)) and alpha > self._min_alpha:
            alpha *= self._delta
            new_x = x + alpha * d
            new_f = problem.evaluateFunctions(new_x)
            f_eval += 1
